...
(function ($) {
	...
	$(document).ready(function () {
		...
		function addWatchedSegment(cookieKey, startTime, endTime, duration) {
			if (!cookieKey || startTime >= endTime) {
				return;
			}
		...
		window.LearnDash_Video_Progress_setSetting = function (
			ld_video_player,
			player_setting_key,
			...
			if (player_setting_key === 'video_time') {
				...
				if (position.isPlaying && position.sessionStart < currentTime) {
					const duration =
						ld_video_player.player_cookie_values.video_duration ||
						0;
					addWatchedSegment(
						cookieKey,
					...
					if (completion.shouldAutoComplete) {
						...
						console.log(
							'Video watched 100% - auto-completing lesson'
						);
			...
			if (player_setting_key === 'video_state') {
				...
				if (player_setting_value === 'play') {
					...
					player_setting_value === 'pause' ||
					player_setting_value === 'complete'
				) {
					if (
						position.isPlaying &&
						position.sessionStart < position.lastPosition
						...
						const duration =
							ld_video_player.player_cookie_values
								.video_duration || 0;
						addWatchedSegment(
							cookieKey,
							position.sessionStart,
					...
					if (ld_video_players.hasOwnProperty(key)) {
						...
						if (player.player_cookie_key) {
							...
							if (completion.shouldEnableButton) {
								...
								if (
									typeof LearnDash_disable_assets !==
									'undefined'
								) {
									LearnDash_disable_assets(false);
								}
					...
					if (ld_video_players.hasOwnProperty(key)) {
						...
						if (player.player_cookie_key) {
							...
							if ($button.length) {
								let $progress = $button
									.parent()
									.find('.ld-video-progress-indicator');
								if (!$progress.length) {
									$progress = $(
										'<div class="ld-video-progress-indicator"></div>'
									).insertAfter($button);
								}
								...
								$progress.html(
									'<span style="color: #666; font-size: 14px;">Video Progress: ' +
										percentage +
										'%' +
										(percentage >= 75
											? ' - You can now mark as complete!'
						...
						for (const key in ld_video_players) {
							if (ld_video_players.hasOwnProperty(key)) {
								...
								if (player.player_cookie_key) {
									...
									if (
										completion.shouldEnableButton &&
										$(
											'form.sfwd-mark-complete .learndash_mark_complete_button'
										).prop('disabled')
									) {
										console.log(
											'Periodic check: Enabling mark complete button at ' +
												Math.round(
													completion.percentage * 100
												) +
												'%'
