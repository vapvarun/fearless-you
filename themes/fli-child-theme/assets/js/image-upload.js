...
jQuery(document).ready(function($) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $(document).trigger('fli_image_uploaded', [response.data]);
                        ...
                        showMessage(response.data.message || fli_upload.upload_error, 'error');
                        resetButtons();
                    }
                },
                error: function() {
    ...
    $('.fli-image-gallery').each(function() {
        ...
        $(document).on('fli_image_uploaded', function(e, data) {
            if (uploadArea.length && uploadArea.closest('.fli-image-gallery').is(gallery)) {
        ...
        gallery.on('click', '.fli-gallery-remove', function(e) {
            ...
            if (confirm('Remove this image from gallery?')) {
                ...
                $(document).trigger('fli_gallery_image_removed', [attachmentId]);
            }
        });
    });
});