...
(function($) {
    ...
    $(document).ready(function() {
        ...
        if (window.fliLearnDashSettings && window.fliLearnDashSettings.showProgress === 'yes') {
            initProgressRings();
            initQuizDisplay();
        }
    });
    ...
    function initProgressRings() {
        ...
        const lessonItems = $('.ld-item-list-item, .ld-table-list-item, .ld-lesson-item');
        ...
        lessonItems.each(function() {
            ...
            if ($statusIcon.hasClass('ld-status-complete')) {
                ...
                const progressText = $item.find('.ld-progress-percentage').text();
                if (progressText) {
    ...
    function createProgressRing($item, status, progress, quizScore) {
        ...
        if (quizScore !== null) {
            // Display quiz score
            const passThreshold = window.fliLearnDashSettings ? window.fliLearnDashSettings.quizPass : 70;
            const scoreClass = quizScore >= passThreshold ? 'score-high' : 'score-low';
            $wrapper.html(`
    ...
    function initQuizDisplay() {
        ...
        if ($quizQuestions.length) {
            ...
            const totalQuestions = $quizQuestions.find('.wpProQuiz_listItem').length;
        ...
        if ($quizHeader.length && !$quizHeader.find('.quiz-skip-button').length) {
            $quizHeader.append('<button class="quiz-skip-button">Skip</button>');
        }
     ...
     */
    window.updateLDProgressRing = function(itemId, status, progress, quizScore) {
