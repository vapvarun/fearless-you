			...
			instance.observe = function (target, options) {
				...
				if (
					...
					try {
						...
						console.warn(
							'MutationObserver.observe error caught:',
							e
						);
					}
				} else {
					console.warn(
						'MutationObserver: Attempted to observe invalid target:',
						target
					);
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function safeClassList(element) {
			if (!element || !element.classList) {
				return {
					add() {
						console.warn('Attempted classList.add on null element');
					},
					remove() {
						console.warn(
							'Attempted classList.remove on null element'
						);
					},
					toggle() {
						console.warn(
							'Attempted classList.toggle on null element'
						);
						return false;
					},
					contains() {
